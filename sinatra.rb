...
class Gallery < Sinatra::Base
  ...
  get '/' do
    ...
    #unless params[:captures].first.empty?
    #  image_index = images.index(params[:captures].first)
    #  image = params[:captures].first
    #else
      image_index = 0 
      image = images.first
    #end
  ...
  get '/:dir/?:image?' do
    ...
    thumbnails = File.directory?("#{img_path}/thumbnails") ? get_images("#{img_path}/thumbnails") : []
